...
const GlobalDropdownSelect = ({ ...props }) => {
  ...
  > = async (searchQuery, _loadedOptions, additional = { page: 1 }) => {
    ...
    try {
      const response = await mainInstance.get(
        `/select/global-dropdowns?page=${page}&search=${searchQuery}&sort=label&module=${props.module}&type=${props.type}`,
      );
...
export default GlobalDropdownSelect;
