...
const UpdateTask = ({ refetch }: UpdateTaskProps) => {
  // Access store values
  const { selectedTask, isOpenUpdateTaskDialog, setIsOpenUpdateTaskDialog } =
    useTaskStore();
  ...
  const onSubmit = (data: z.infer<typeof FormSchema>) => {
    ...
    toast.promise(mainInstance.patch(`/tasks/${selectedTask?.id}`, data), {
      ...
      success: () => {
        refetch();
        setIsOpenUpdateTaskDialog(false);
        return 'Success!';
      },
      error: error => {
  ...
  return (
    <Dialog
      open={isOpenUpdateTaskDialog}
      onOpenChange={setIsOpenUpdateTaskDialog}
    >
      <DialogContent>
        {/* Form */}
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} autoComplete="off">
            ...
            <DialogFooter className="flex justify-end gap-2">
              <Button
                variant="outline"
                onClick={() => setIsOpenUpdateTaskDialog(false)}
              >
                Close
              </Button>
