...
const DeleteTask = ({ refetch }: DeleteTaskProps) => {
  // Access store values
  const { selectedTask, isOpenDeleteTaskDialog, setIsOpenDeleteTaskDialog } =
    useTaskStore();
  ...
  const onSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    toast.promise(mainInstance.delete(`/tasks/${selectedTask?.id}`), {
      loading: 'Loading...',
      success: () => {
        refetch();
  ...
  return (
    <Dialog
      open={isOpenDeleteTaskDialog}
      onOpenChange={setIsOpenDeleteTaskDialog}
    >
      <DialogContent>
        {/* Form */}
        <form onSubmit={onSubmit}>
          ...
          <DialogFooter className="flex justify-end gap-2">
            <Button
              variant="outline"
              onClick={() => setIsOpenDeleteTaskDialog(false)}
            >
              Cancel
            </Button>
            <Button
